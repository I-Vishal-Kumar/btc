...
export const USDT = async (
    ...
    try {
        ...
        if (amount < 100)
            throw new Error(`Minimum amount is $${(100 / 89).toFixed(2)}.`);
...
export const CREATE_TRANSACTION = async (
    ...
    try {
        ...
        if (amount < 500) throw new Error("Minimum amount is 100.");
...
export const getTransactionDetails = async (
    ...
    try {
        ...
        const depositDetails = await TRANSACTION.find(
            { PhoneNumber: decoded.PhoneNumber, Type: { $in: [...type] } },
            { Parent: 0, InvitationCode: 0, PhoneNumber: 0 }
        )
            .sort({ createdAt: -1 })
            .limit(50);
...
export const getIncomeHistory = async (): ServiceReturnType<Income[]> => {
    try {
        ...
        return {
            valid: true,
            data: (incomeDetails as unknown as Income[]) || [],
        };
    } catch (error) {
        if (!(error instanceof Error))
            return {
