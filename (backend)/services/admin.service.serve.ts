...
import {
    AdminConfigType,
    ad_getUserInfoResType,
} from "@/__types__/admin.types";
...
import {
    IncomeType,
    TransactionObjType,
    adminWithdrawalRespType,
} from "@/__types__/transaction.types";
...
export const verifyAdminPass = async (Password: string): ServiceReturnType => {
    try {
        ...
        if (isCorrectPass)
            return { valid: true, msg: "Logged in successfully" };
...
export const getAdminConfig = async (): ServiceReturnType<AdminConfigType> => {
    try {
        ...
        const adminConfig = await ADMIN_CONFIG.find(
            {},
            { _id: 0, Password: 0 }
        );
        ...
        return {
            valid: true,
            data: adminConfig[0].toObject() as AdminConfigType,
        };
    } catch {
...
export const ad_getUserInfo = async ({
    PhoneNumber,
}: {
    PhoneNumber: string;
}): ServiceReturnType<ad_getUserInfoResType> => {
    try {
        await CONNECT();
        ...
        const userDetails = await USER.aggregate([
            ...
            },
            {
                $addFields: {
                    ...
                    ParentPhoneNumber: {
                        $ifNull: [
                            { $arrayElemAt: ["$parentDetails.PhoneNumber", 0] },
                            null,
                        ],
                    },
                },
        ...
        return {
            valid: true,
            data: JSON.parse(JSON.stringify({ ...userDetails[0], ...details })),
        };
    } catch (error) {
        console.log(error);
...
export const ad_editAdminConfig = async ({
    key,
    newVal,
}: argsType): ServiceReturnType<argsType> => {
    try {
...
export const ad_getAdminConfig =
    async (): ServiceReturnType<AdminConfigType> => {
        try {
            await CONNECT();
...
export const ad_getDepositTransactions = async ({
    TransactionID = "",
    page = 1,
    ...
    try {
        ...
        if (TransactionID) {
            const details: TransactionObjType[] | null =
...
export const ad_settleDeposit = async (
    ...
    try {
        if (editedDetails.Status === TransactionStatusType.PENDING)
            throw new Error("Please choose correct type.");
        ...
        const isTransactionUpdated = await TRANSACTION.findByIdAndUpdate(
            editedDetails._id,
            {
                Amount: amount,
        ...
        if (!user.Deposited && user.Parent) {
            ...
            const parentDetails: UserType | null = await USER.findOne({
        ...
        // Perform bulk update
        const bulkWriteResult = await USER.bulkWrite(userUpdateOperations, {
            session,
        });
        ...
        if (bulkWriteResult.modifiedCount !== userUpdateOperations.length) {
            throw new Error(
                "Mismatch in expected updates. Transaction aborted."
            );
        }
...
export const ad_getWithdrawalTransactions = async ({
    TransactionID = "",
    page = 1,
}): ServiceReturnType<adminWithdrawalRespType[]> => {
    try {
        await CONNECT();

        if (TransactionID) {
            const details: adminWithdrawalRespType[] | null =
...
export const ad_settleWithdrawal = async (
    editedDetails: TransactionObjType
): ServiceReturnType => {
    const session = await startSession();
    session.startTransaction();
    ...
    try {
        ...
        if (editedDetails.Status === TransactionStatusType.FAILED) {
            ...
            const isUpdated = await USER.findOneAndUpdate(
                ...
                {
                    $inc: {
                        Balance: Number(editedDetails.Amount) + taxableAmount,
                    },
                },
                { session }
            );
...
export const ad_getUserWallet = async ({
    PhoneNumber,
}: {
    PhoneNumber: string;
}): ServiceReturnType<UserWallet> => {
    try {
        await CONNECT();

        const details: UserWallet[] | null = await WALLET.findOne(
            { PhoneNumber },
            { updatedAt: 0 }
        );
...
export const ad_editUserWallet = async (
    editedDetails: UserWallet
): ServiceReturnType => {
    try {
        await CONNECT();

        const isUpdated: UserWallet[] | null = await WALLET.findOneAndUpdate(
            { PhoneNumber: editedDetails.PhoneNumber },
            {
                $set: {
                    ...editedDetails,
...
export const ad_getUserPassInfo = async ({
    PhoneNumber,
}: {
    PhoneNumber: string;
    ...
    try {
        ...
        const details: UserType | null = await USER.findOne(
            { PhoneNumber },
            { PhoneNumber: 1, Password: 1, Name: 1, Blocked: 1 }
        );
...
export const ad_editPassword = async (
    editedDetails: UserType
): ServiceReturnType => {
    try {
        await CONNECT();

        const isUpdated: UserWallet[] | null = await USER.findOneAndUpdate(
            { PhoneNumber: editedDetails.PhoneNumber },
            {
...
export const ad_getBlockedUsers = async (): ServiceReturnType<UserType[]> => {
    try {
        ...
        const details: UserType[] | null = await USER.find(
            { Blocked: true },
            { PhoneNumber: 1, Name: 1, Blocked: 1 }
        );
...
export const ad_blockUnblock = async (
    editedDetails: UserType
): ServiceReturnType => {
    try {
        await CONNECT();
...
export const ad_getPeningVideoApproval = async (): ServiceReturnType<
    VideoType[]
> => {
    try {
        await CONNECT();
...
export const ad_getAllApprovedVideos = async (): ServiceReturnType<
    VideoType[]
> => {
    try {
        await CONNECT();
...
export const ad_toggleVideoVisibility = async ({
    id,
    newStatus,
}: {
    newStatus: VissibilityStatusType;
    id: string;
}): Promise<boolean> => {
    try {
...
export const ad_videoRejectApprove = async ({
    id,
    operation,
    rewardAmount,
    durationInSec,
    ...
    try {
        ...
        if (!durationInSec || durationInSec <= 1)
            throw new Error(
                "Failed to approve please select a valid duration."
            );

        if (!rewardAmount || rewardAmount <= 1)
            throw new Error("Please select a valid reward ammount");

        const videoUpdated = await VIDEOS.findOneAndUpdate(
            { _id: new mongoose.Types.ObjectId(id) },
            {
                $set: {
                    ApprovalStatus: VideoApprovalStatusType.APPROVED,
        ...
        await USER.findOneAndUpdate(
            ...
            {
                $inc: {
                    Balance: Number(rewardAmount),
                    Profit: Number(rewardAmount),
                },
            },
            { session }
        );
